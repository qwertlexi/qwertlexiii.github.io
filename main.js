(function () {
  ...
  var radioLabelsEl = document.getElementById("radio-window-labels");
  var radioStatusEl = document.getElementById("radio-status");
  var radioKnob     = document.getElementById("radio-knob");
  var radioKnobNotch= radioKnob ? radioKnob.querySelector(".radio-knob-notch") : null;
  var ctrlRadio     = document.getElementById("ctrl-radio");

  var RADIO_MIN = 87.5, RADIO_MAX = 108.0, RADIO_STEP = 0.1, RADIO_LOCK = 0.05;
  // Inset the needle's travel a few % on each side so it never sits flush
  // against the dial window's edge at the band extremes.
  var RADIO_INSET = 3;
  ...
  function freqToPct(f) {
    var raw = (f - RADIO_MIN) / (RADIO_MAX - RADIO_MIN) * 100;
    return RADIO_INSET + raw * (100 - RADIO_INSET * 2) / 100;
  }

  if (radioLabelsEl) {
    [88, 92, 96, 100, 104, 108].forEach(function(f) {
      var lbl = document.createElement("span");
      lbl.className = "radio-tick-lbl";
      lbl.style.left = freqToPct(f) + "%";
      lbl.textContent = f;
      radioLabelsEl.appendChild(lbl);
    });
  }

  function renderRadio() {
    if (radioFreqEl) radioFreqEl.textContent = radioFreq.toFixed(1);
    var pct = freqToPct(radioFreq);
    if (radioNeedle) radioNeedle.style.left = pct + "%";
    if (radioKnobNotch) {
      var sweepPct = (radioFreq - RADIO_MIN) / (RADIO_MAX - RADIO_MIN) * 100;
      var deg = -150 + (sweepPct / 100) * 300; // sweep ~300° across the dial range
      radioKnobNotch.style.transform = "translate(-1px,-1.1rem) rotate(" + deg + "deg)";
    }
    var hit = null;
  ...
  // ── Rotary knob: drag (mouse/touch) to tune, like turning a real dial ──
  if (radioKnob) {
    radioKnob.setAttribute("tabindex", "0");
    radioKnob.addEventListener("keydown", function(e) {
      if (e.key === "ArrowLeft")  { e.preventDefault(); radioStep(-1); }
      else if (e.key === "ArrowRight") { e.preventDefault(); radioStep(1); }
    });

    var dragging = false, lastX = 0, lastY = 0, accum = 0;

    function knobMove(x, y) {
      var dx = x - lastX, dy = y - lastY;
      lastX = x; lastY = y;
      // Combine horizontal drag + vertical drag (like turning a knob with mouse motion)
      accum += dx - dy;
      var STEP_PX = 6; // px of drag per 0.1 MHz step
      while (accum >= STEP_PX) { radioStep(1); accum -= STEP_PX; }
      while (accum <= -STEP_PX) { radioStep(-1); accum += STEP_PX; }
    }

    radioKnob.addEventListener("mousedown", function(e) {
      dragging = true; lastX = e.clientX; lastY = e.clientY; accum = 0;
      e.preventDefault();
    });
    window.addEventListener("mousemove", function(e) {
      if (!dragging) return;
      knobMove(e.clientX, e.clientY);
    });
    window.addEventListener("mouseup", function() { dragging = false; });

    radioKnob.addEventListener("touchstart", function(e) {
      var t = e.touches[0];
      dragging = true; lastX = t.clientX; lastY = t.clientY; accum = 0;
    }, { passive: true });
    radioKnob.addEventListener("touchmove", function(e) {
      if (!dragging) return;
      var t = e.touches[0];
      knobMove(t.clientX, t.clientY);
      e.preventDefault();
    }, { passive: false });
    radioKnob.addEventListener("touchend", function() { dragging = false; });
  }

  // ── Speaker grille EQ: bars pulse with the beat while a track plays ──
  var radioEqEl = document.getElementById("radio-eq");
  var radioEqBars = [];
  var RADIO_EQ_N = 14;
  if (radioEqEl) {
    for (var rqi = 0; rqi < RADIO_EQ_N; rqi++) {
      var bar = document.createElement("span");
      radioEqEl.appendChild(bar);
      radioEqBars.push({ el: bar, jitter: Math.random(), phase: Math.random() * Math.PI * 2 });
    }
  }

  var radioEqRaf = null, radioEqT = 0;
  function radioEqTick() {
    radioEqRaf = requestAnimationFrame(radioEqTick);
    radioEqT += 0.06;
    var playing = !audio.paused && radioOpen;
    initBeatAnalyser();
    var energy = playing ? getBeatEnergy() : 0;
    radioEqBars.forEach(function(b, i) {
      var base = playing
        ? 14 + energy * 78 + Math.sin(radioEqT * 2.4 + b.phase + i * 0.5) * 10 * (0.4 + energy)
        : 8 + Math.sin(radioEqT * 0.6 + b.phase) * 2;
      var h = Math.max(6, Math.min(100, base + (Math.random() - 0.5) * (playing ? 14 : 2)));
      b.el.style.height = h + "%";
    });
  }
  function startRadioEq() { if (!radioEqRaf) radioEqTick(); }
  function stopRadioEq()  { if (radioEqRaf) { cancelAnimationFrame(radioEqRaf); radioEqRaf = null; } }
