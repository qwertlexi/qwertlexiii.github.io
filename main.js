(function () {
  ...
  var radioScaleEl  = document.getElementById("radio-window-scale");
  ...
  if (radioScaleEl) {
    // Bare tick marks via CSS repeating-gradient (already on .radio-window-scale);
    // overlay numeric MHz labels at a few reference points, plus a marker per station.
    [88, 92, 96, 100, 104, 108].forEach(function(f) {
      var pct = (f - RADIO_MIN) / (RADIO_MAX - RADIO_MIN) * 100;
      var lbl = document.createElement("span");
      lbl.className = "radio-tick-lbl";
      lbl.style.left = pct + "%";
      lbl.textContent = f;
      radioScaleEl.appendChild(lbl);
    });
  }
  ...
  function renderRadio() {
    ...
    if (radioKnobNotch) {
      var deg = -150 + (pct / 100) * 300; // sweep ~300° across the dial range
